...
type MobileTab = "sources" | "chat" | "tools";
type ColumnKey = "sources" | "chat" | "tools";
...
function loadCollapsed(): Record<ColumnKey, boolean> {
  ...
  return { sources: false, chat: false, tools: false };
}

function buildGridCols(collapsed: Record<ColumnKey, boolean>): string {
  const cols: string[] = [
    collapsed.sources ? "40px" : "1fr",
    collapsed.chat ? "40px" : "1fr",
    collapsed.tools ? "40px" : "1fr",
  ];
  return cols.join(" ");
}
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex md:hidden border-b border-stone-800 shrink-0">
        {(["sources", "chat", "tools"] as MobileTab[]).map(tab => (
          <button
            key={tab}
      ...
      <div className="flex-1 overflow-hidden">
        {/* Desktop */}
        <div
          className="hidden md:grid h-full workspace-grid"
        ...
        >
          ...
          <ChatPanel collapsed={collapsed.chat} onToggleCollapse={() => toggleCollapse("chat")} />
          <ToolsPanel collapsed={collapsed.tools} onToggleCollapse={() => toggleCollapse("tools")} />
        </div>
